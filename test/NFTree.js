...
describe("NFTree Contract", function () {
  ...
  describe("Minting a tree", function () {
    ...
    it("should fail to mint a tree with an already used tokenId", async function () {
      ...
      await nftree.connect(owner).mint(user1.address, tokenId, "Oak", 10, "USA", "http://example.com/tree.jpg");
  
      await expect(nftree.connect(owner).mint(user1.address, tokenId, "Pine", 5, "Kenya", "http://example.com/tree2.jpg")).to.be.reverted;
    });
    ...
    it("should fail to mint a tree by a non-owner", async function () {
      ...
      await expect(nftree.connect(user1).mint(user1.address, tokenId, species, age, location, url))
  ...
  describe("Fetching tree info", function () {
    ...
    it("should transfer a tree from one user to another", async function () {
      ...
      await nftree.connect(user1).transferFrom(user1.address, owner.address, tokenId);
  ...
  describe("Fetching tree URL", function () {
    ...
    it("should burn a tree", async function () {
      const tokenId = 1;
      
      await nftree.connect(user1).burn(tokenId);
      
      await expect(nftree.getTreeInfo(tokenId)).to.be.revertedWith("Token ID does not exist");
    });
  ...
  describe("Fetching total supply", function () {
    ...
    it("should return correct total supply after minting multiple trees", async function () {
      await nftree.connect(owner).mint(user1.address, 1, "Oak", 5, "Kenya", "http://example.com/tree1.jpg");
      await nftree.connect(owner).mint(user1.address, 2, "Pine", 10, "USA", "http://example.com/tree2.jpg");
